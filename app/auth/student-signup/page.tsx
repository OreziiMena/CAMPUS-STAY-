...
function StudentSignupContent() {
  ...
  // Auto-fill and validate referral code from query params e.g. ?ref=CT-EMEN-924
  useEffect(() => {
  ...
  const validateCode = async (code: string) => {
    ...
    try {
      ...
      if (res.valid) {
        ...
        setReferralMessage(res.message || "Valid referral code!");
      } else {
        setReferralStatus("invalid");
        setReferralMessage(res.message || "Invalid referral code.");
      }
    } catch {
      setReferralStatus("idle");
    }
            ...
            <form id="student-signup-form" className="auth-form" onSubmit={handleSubmit}>
              ...
              <div className="input-group">
                <label htmlFor="referralCode">
                  Referral Code <span className="optional-badge">(Optional)</span>
                </label>
                <input
                  type="text"
                  id="referralCode"
                  placeholder="e.g. CT-CAMP-123"
                  value={referralCode}
                  onChange={(e) => {
                ...
                {referralStatus === "validating" && (
                  <small className="helper-text">
                    <i className="fas fa-spinner fa-spin"></i> Checking referral code...
                  </small>
                )}
                {referralStatus === "valid" && (
                  <small className="success-text validation-success referral-valid-text">
                    <i className="fas fa-check-circle"></i> {referralMessage}
                  </small>
                )}
                {referralStatus === "invalid" && (
                  <small className="error-text validation-error">
                    <i className="fas fa-exclamation-triangle"></i> {referralMessage}
                  </small>
                )}
              </div>
...
export default function StudentSignup() {
  return (
    <Suspense fallback={<div className="auth-page"><div className="auth-container"><p className="suspense-loading-text">Loading...</p></div></div>}>
